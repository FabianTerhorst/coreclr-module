...
var ASM_CONSTS = {
  567736: function() {return STACK_MAX;},  
 567758: function() {return TOTAL_STACK;},  
 641662: function($0, $1) {var str = UTF8ToString ($0); try { var res = eval (str); if (res === null || res == undefined) return 0; res = res.toString (); setValue ($1, 0, "i32"); } catch (e) { res = e.toString (); setValue ($1, 1, "i32"); if (res === null || res === undefined) res = "unknown exception"; } var buff = Module._malloc((res.length + 1) * 2); stringToUTF16 (res, buff, (res.length + 1) * 2); return buff;},  
 642058: function() {var err = new Error(); out ("Stacktrace: \n"); out (err.stack);}
};
...
// {{PRE_LIBRARY}}
  ...
  var MONO={pump_count:0,timeout_queue:[],mono_wasm_runtime_is_ready:false,mono_wasm_ignore_pdb_load_errors:true,pump_message:function () {
  			...
  			this.var_info = [];
  			out (">> mono_wasm_get_object_properties " + objId);
  			...
  			this.var_info = [];
  			out (">> mono_wasm_get_array_values " + objId);
  			...
  			out (">> mono_wasm_start_single_stepping " + kind);
  			...
  			file_list.forEach (function(file_name) {
  				...
  				fetch_promise.then (function (response) {
  					...
  					out ("MONO_WASM: Loaded: " + file_name);
  					--pending;
  					if (pending == 0) {
  						MONO.loaded_files = loaded_files;
  						var load_runtime = Module.cwrap ('mono_wasm_load_runtime', null, ['string', 'number']);
  
  						out ("MONO_WASM: Initializing mono runtime");
  						if (ENVIRONMENT_IS_SHELL || ENVIRONMENT_IS_NODE) {
  							try {
  								load_runtime (vfs_prefix, enable_debugging);
  							} catch (ex) {
  								out ("MONO_WASM: load_runtime () failed: " + ex);
  								var err = new Error();
  								out ("MONO_WASM: Stacktrace: \n");
  								out (err.stack);
  			...
  			out(">>>mono_wasm_get_loaded_files");
  ...
  function _mono_wasm_add_func_var(className, objectId) {
  		if (objectId == 0) {
  			MONO.mono_wasm_add_null_var (className);
  			return;
  		}
...
if (ENVIRONMENT_IS_NODE) {
  ...
  } else _emscripten_get_now = function() { return /*performance['now']()*/Date.now(); };
  ;
