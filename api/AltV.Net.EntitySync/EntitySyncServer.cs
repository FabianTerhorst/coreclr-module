...
namespace AltV.Net.EntitySync
{
    ...
    public class EntitySyncServer
    {
        ...
        public EntitySyncServer(ulong threadCount, Func<ulong, int> syncRate, Func<ulong, bool> netOwnerEvents,
            ...
            Func<ulong, SpatialPartition> createSpatialPartition, IIdProvider<ulong> idProvider)
        {
            ...
            for (ulong i = 0; i < threadCount; i++)
            {
                ...
                var threadSyncRate = syncRate(i);
                if (threadSyncRate < 0)
                {
                    throw new ArgumentException("syncRate must be >= 0 in thread " + i);
                }
                ...
                entityThreads[i] = new EntityThread(i, entityThreadRepository, clientThreadRepository, spatialPartition, threadSyncRate, netOwnerEvents(i),
                    OnEntityCreate,
        ...
        {
            entityRepository.Remove(id, type);
            idProvider?.Free(id);
        }
