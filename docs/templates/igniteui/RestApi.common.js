...
exports.transform = function(model) {
    var _fileNameWithoutExt = common.path.getFileNameWithoutExtension(
        model._path
    );
    ...
    model.docurl =
        model.docurl ||
        common.getImproveTheDocHref(
            model,
            model._gitContribute,
            model._gitUrlPattern
        );
    model.sourceurl =
        model.sourceurl ||
        common.getViewSourceHref(model, null, model._gitUrlPattern);
    ...
    if (model.children) {
        for (var i = 0; i < model.children.length; i++) {
            ...
            child.sourceurl =
                child.sourceurl ||
                common.getViewSourceHref(child, null, model._gitUrlPattern);
            ...
            for (var j = examples.length - 1; j >= 0; j--) {
                ...
                if (mimeType === 'application/json') {
                    try {
                        ...
                        responses[i].examples[j].content = JSON.stringify(
                            json,
                            null,
                            '  '
                        );
    ...
    function transformPropertiesValue(obj) {
        if (obj.type === 'array' && obj.items) {
            ...
            obj.items['x-internal-ref-name'] =
                obj.items['x-internal-ref-name'] || null;
            obj.items['x-internal-loop-ref-name'] =
                obj.items['x-internal-loop-ref-name'] || null;
            ...
            obj.items['x-internal-ref-name'] =
                obj['x-internal-ref-name'] || null;
            obj.items['x-internal-loop-ref-name'] =
                obj['x-internal-loop-ref-name'] || null;
            transformProperties(obj.items);
        }
    }
    ...
    function appendQueryParamsToPath(path, parameters) {
        ...
        var requiredQueryParams = parameters.filter(function(p) {
            return p.in === 'query' && p.required;
        });
        if (requiredQueryParams.length > 0) {
            path = formatParams(path, requiredQueryParams, true);
        }

        var optionalQueryParams = parameters.filter(function(p) {
            return p.in === 'query' && !p.required;
        });
...
exports.getBookmarks = function(model) {
    ...
    if (model.tags) {
        model.tags.forEach(function(tag) {
            if (tag.uid) {
                bookmarks[tag.uid] = tag['x-bookmark-id']
                    ? tag['x-bookmark-id']
                    : common.getHtmlId(tag.uid);
            }
            if (tag.children) {
